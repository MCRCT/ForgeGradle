package ch.minecraftreunitedcollaborationteam.gradle.user;

import static ch.minecraftreunitedcollaborationteam.gradle.common.Constants.FERNFLOWER;
import static ch.minecraftreunitedcollaborationteam.gradle.common.Constants.JAR_CLIENT_FRESH;
import static ch.minecraftreunitedcollaborationteam.gradle.common.Constants.JAR_MERGED;
import static ch.minecraftreunitedcollaborationteam.gradle.common.Constants.JAR_SERVER_FRESH;
import static ch.minecraftreunitedcollaborationteam.gradle.user.UserConstants.*;

import ch.minecraftreunitedcollaborationteam.gradle.json.version.Library;
import ch.minecraftreunitedcollaborationteam.gradle.tasks.GenSrgTask;
import ch.minecraftreunitedcollaborationteam.gradle.tasks.abstractutil.ExtractTask;
import ch.minecraftreunitedcollaborationteam.gradle.tasks.user.SourceCopyTask;
...
import ch.minecraftreunitedcollaborationteam.gradle.common.BasePlugin;
...
import ch.minecraftreunitedcollaborationteam.gradle.json.JsonFactory;
...
import ch.minecraftreunitedcollaborationteam.gradle.tasks.ExtractConfigTask;
import ch.minecraftreunitedcollaborationteam.gradle.tasks.GenSrgTask;
import ch.minecraftreunitedcollaborationteam.gradle.tasks.MergeJarsTask;
...
import ch.minecraftreunitedcollaborationteam.gradle.tasks.abstractutil.ExtractTask;
import ch.minecraftreunitedcollaborationteam.gradle.tasks.user.SourceCopyTask;
import ch.minecraftreunitedcollaborationteam.gradle.tasks.user.reobf.ArtifactSpec;
import ch.minecraftreunitedcollaborationteam.gradle.tasks.user.reobf.ReobfTask;
...
//    protected abstract void configureDecompSetup(Task task);
    ...
    public String resolve(String pattern, Project project, T exten)
    {
        ...
        pattern = pattern.replace("{MCP_DATA_DIR}", UserConstants.CONF_DIR);
    ...
    protected void configureDeps()
    {
        ...
        project.getConfigurations().create(UserConstants.CONFIG_START);
        project.getConfigurations().create(UserConstants.CONFIG_DEPS);
        project.getConfigurations().create(UserConstants.CONFIG_MC);
        ...
        extractUserDev.setConfig(UserConstants.CONFIG_USERDEV);
        ...
        extractUserDev.doLast(new Action<Task>()
        {
            ...
            {
                readAndApplyJson(getDevJson().call(), UserConstants.CONFIG_DEPS, UserConstants.CONFIG_NATIVES, arg0.getLogger());
            }
        });
        ...
        extractNatives.setConfig(UserConstants.CONFIG_NATIVES);
        ...
        project.getDependencies().add(UserConstants.CONFIG_START, project.files(delayedFile(getStartDir())));

        // extra libs folder.
        project.getDependencies().add("compile", project.fileTree("libs"));

        // make MC dependencies into normal compile classpath
        project.getDependencies().add("compile", project.getConfigurations().getByName(UserConstants.CONFIG_DEPS));
        project.getDependencies().add("compile", project.getConfigurations().getByName(UserConstants.CONFIG_MC));
        project.getDependencies().add("runtime", project.getConfigurations().getByName(UserConstants.CONFIG_START));
    }
    ...
    public final void injectIntellijRuns(Document doc, String module) throws DOMException, IOException
    {
        ...
        String[][] config = new String[][]
        {
                ...
                {
                        "Minecraft Client",
                        UserConstants.GRADLE_START_CLIENT,
                ...
                {
                        "Minecraft Server",
                        UserConstants.GRADLE_START_SERVER,
    ...
    {
        {
            ...
            task.setInSrg(delayedFile(UserConstants.PACKAGED_SRG));
            task.setInExc(delayedFile(UserConstants.PACKAGED_EXC));
            task.setMethodsCsv(delayedFile(UserConstants.METHOD_CSV));
            task.setFieldsCsv(delayedFile(UserConstants.FIELD_CSV));
            task.setNotchToSrg(delayedFile(UserConstants.DEOBF_SRG_SRG));
            task.setNotchToMcp(delayedFile(UserConstants.DEOBF_MCP_SRG));
            task.setSrgToMcp(delayedFile(UserConstants.DEOBF_SRG_MCP_SRG));
            task.setMcpToSrg(delayedFile(UserConstants.REOBF_SRG));
            task.setMcpToNotch(delayedFile(UserConstants.REOBF_NOTCH_SRG));
            task.setSrgExc(delayedFile(UserConstants.EXC_SRG));
            task.setMcpExc(delayedFile(UserConstants.EXC_MCP));
...

        {
            ...
            task.setSrg(delayedFile(UserConstants.DEOBF_MCP_SRG));
            task.setExceptorJson(delayedFile(UserConstants.EXC_JSON));
            task.setExceptorCfg(delayedFile(UserConstants.EXC_MCP));
            task.setFieldCsv(delayedFile(UserConstants.FIELD_CSV));
            task.setMethodCsv(delayedFile(UserConstants.METHOD_CSV));
            task.setInJar(delayedFile(JAR_MERGED));
            task.setOutCleanJar(delayedFile("{API_CACHE_DIR}/" + UserConstants.MAPPING_APPENDAGE + name));
            task.setOutDirtyJar(delayedFile(UserConstants.DIRTY_DIR + "/" + name));
        ...
        {
            String name = "{API_NAME}-" + (hasApiVersion() ? "{API_VERSION}" : "{MC_VERSION}") + "-"+ UserConstants.CLASSIFIER_DEOBF_SRG +".jar";

            ProcessJarTask task = makeTask("deobfuscateJar", ProcessJarTask.class);
            task.setSrg(delayedFile(UserConstants.DEOBF_SRG_SRG));
            task.setExceptorJson(delayedFile(UserConstants.EXC_JSON));
            task.setExceptorCfg(delayedFile(UserConstants.EXC_SRG));
            task.setInJar(delayedFile(JAR_MERGED));
...

        {
            ...
            task.setExceptorCfg(delayedFile(UserConstants.EXC_SRG));
            task.setSrg(delayedFile(UserConstants.REOBF_SRG));
            task.setFieldCsv(delayedFile(UserConstants.FIELD_CSV));
            task.setFieldCsv(delayedFile(UserConstants.METHOD_CSV));
            task.setMcVersion(delayedString("{MC_VERSION}"));
...

        {
            ...
            task.compileResources(UserConstants.CONFIG_DEPS);
    ...
    {
        DelayedFile decompOut = delayedDirtyFile(null, UserConstants.CLASSIFIER_DECOMPILED, "jar", false);
        DelayedFile remapped = delayedDirtyFile(getSrcDepName(), UserConstants.CLASSIFIER_SOURCES, "jar");
        final DelayedFile recomp = delayedDirtyFile(getSrcDepName(), null, "jar");
        final DelayedFile recompSrc = delayedFile(UserConstants.RECOMP_SRC_DIR);
        final DelayedFile recompCls = delayedFile(UserConstants.RECOMP_CLS_DIR);

        DecompileTask decomp = makeTask("decompile", DecompileTask.class);
        {
            decomp.setInJar(delayedDirtyFile(null, UserConstants.CLASSIFIER_DEOBF_SRG, "jar", false));
            decomp.setOutJar(decompOut);
            decomp.setFernFlower(delayedFile(FERNFLOWER));
            decomp.setPatch(delayedFile(UserConstants.MCP_PATCH_DIR));
            decomp.setAstyleConfig(delayedFile(UserConstants.ASTYLE_CFG));
        ...
        JavaCompile recompTask = makeTask("recompMinecraft", JavaCompile.class);
        {
            ...
            recompTask.setClasspath(project.getConfigurations().getByName(UserConstants.CONFIG_DEPS));
    ...
    private void createExecTasks()
    {
        JavaExec exec = makeTask("runClient", JavaExec.class);
        {
            ...
            exec.setMain(UserConstants.GRADLE_START_CLIENT);
        ...
        {
            exec.doFirst(new MakeDirExist(delayedFile("{RUN_DIR}")));
            exec.setMain(UserConstants.GRADLE_START_SERVER);
    ...
    private final void createSourceCopyTasks()
    {
        ...
        // main
        {
            DelayedFile dir = delayedFile(UserConstants.SOURCES_DIR + "/java");
        ...
        if (project.getPlugins().hasPlugin("scala"))
        {
            ...
            DelayedFile dir = delayedFile(UserConstants.SOURCES_DIR + "/scala");
        ...
        if (project.getPlugins().hasPlugin("groovy"))
        {
            ...
            DelayedFile dir = delayedFile(UserConstants.SOURCES_DIR + "/groovy");
    ...
    public final void afterEvaluate()
    {
        ...
        project.getDependencies().add(UserConstants.CONFIG_USERDEV, delayedString(getUserDev()).call() + ":userdev");

        // grab the json && read dependencies
        if (getDevJson().call().exists())
        {
            readAndApplyJson(getDevJson().call(), UserConstants.CONFIG_DEPS, UserConstants.CONFIG_NATIVES, project.getLogger());
        }
        ...
        final File decompFile = delayedDirtyFile(getSrcDepName(), UserConstants.CLASSIFIER_SOURCES, "jar").call();
    ...
    protected void delayedTaskConfig()
    {
        ...
        // configure output of recompile task
        {
            ...
            compile.setDestinationDir(delayedFile(UserConstants.RECOMP_CLS_DIR).call());
        }
    ...
    protected void setMinecraftDeps(boolean decomp, boolean remove)
    {
        ...
        if (decomp)
        {
            project.getDependencies().add(UserConstants.CONFIG_MC, ImmutableMap.of("name", getSrcDepName(), "version", version));
            if (remove)
            {
                project.getConfigurations().getByName(UserConstants.CONFIG_MC).exclude(ImmutableMap.of("module", getBinDepName()));
            }
        }
        else
        {
            project.getDependencies().add(UserConstants.CONFIG_MC, ImmutableMap.of("name", getBinDepName(), "version", version));
            if (remove)
            {
                project.getConfigurations().getByName(UserConstants.CONFIG_MC).exclude(ImmutableMap.of("module", getSrcDepName()));
            }
        }
    }
    ...
    protected DelayedFile delayedDirtyFile(final String name, final String classifier, final String ext, final boolean usesMappings)
    {
        return new DelayedFile(project, "", this) {
            ...
            public File resolveDelayed()
            {
                ...
                pattern = (decompDeobf.isClean() ? "{API_CACHE_DIR}/"+(usesMappings ? UserConstants.MAPPING_APPENDAGE : "") : UserConstants.DIRTY_DIR) + "/";
